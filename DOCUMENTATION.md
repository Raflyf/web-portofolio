...
| **6.6.0** | 2026-08-17 | Diagnostik Error Backend Dinamis & Penyediaan Kunci Gateway Multilateral (*Dynamic Backend Error Reporting & Active Fallback Injection*): Menghapus penyamaran pesan error generik "kendala jaringan" dan menggantinya dengan pelaporan diagnostik teknis transparan (status HTTP, model target, kegagalan provider) serta menyuntikkan fallback API Key aktif pada backend `/api/chat` agar seluruh permintaan model AI (OpenRouter, Nvidia NIM, OpenCode, MiniMax) selalu terotentikasi secara andal. |
