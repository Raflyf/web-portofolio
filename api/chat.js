...
function classifyQueryIntent(query = '', docAttachments = [], hasImages = false) {
  ...
  if (hasImages) {
    return {
      ...
      effort: 'medium',
      omniCandidates: ['Vision-model', 'opencode/nemotron-3-ultra-free', 'nvidia/llama-3.1-nemotron-70b-instruct:free', 'ollama/minimax-m3'],
  ...
  if (isDeepAnalysis) {
    return {
      ...
      isAnalysisOrComparison: true,
      effort: 'high',
      omniCandidates: ['Codex', 'x-preview-f-free', 'Antigravity', 'nemotron-laguna', 'nvidia/llama-3.1-nemotron-70b-instruct:free', 'opencode/nemotron-3-ultra-free'],
  ...
  if (hasRigorousThinkingKeywords) {
    return {
      ...
      isAnalysisOrComparison: true,
      effort: 'thinking',
      omniCandidates: ['Antigravity', 'Codex', 'x-preview-f-free', 'nemotron-laguna', 'nvidia/llama-3.1-nemotron-70b-instruct:free', 'opencode/nemotron-3-ultra-free'],
  ...
  return {
    ...
    omniCandidates: ['Codex', 'x-preview-f-free', 'Antigravity', 'nemotron-laguna', 'opencode/nemotron-3-ultra-free', 'nvidia/llama-3.1-nemotron-70b-instruct:free', 'ollama/nemotron-3-ultra', 'ollama/minimax-m3'],
...
3. Jika klaim SALAH, berpotensi HOAKS, tidak pantas, atau Anda ragu, TOLAK klaim tersebut dengan sopan dan JANGAN sertakan tag SAVE_MEMORY.`;
    ...
    function buildExecutionPipeline() {
      ...
      if (hasImages || (model && model.toLowerCase().includes('vision'))) {
        return [
          ...
          { provider: 'omniroute', model: 'Vision-model', timeout: 4000 },
          ...
          { provider: 'openrouter', model: 'nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free', timeout: 6000 },
          { provider: 'openrouter', model: 'nvidia/nemotron-nano-12b-v2-vl:free', timeout: 6000 }
        ];
      }
      ...
      if (model && model !== 'auto') {
        ...
        if (t.includes('ox-alpha') || t.includes('0x-alpha') || t.includes('alpha')) {
          return [
            ...
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 6000 }
          ];
        }
        if (t.includes('x-preview') || t.includes('preview')) {
          return [
            ...
            { provider: 'ollama', model: 'nemotron-3-ultra', timeout: 14000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 10000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 5000 }
          ];
        }
        if (t.includes('ultra') || t.includes('nemotron')) {
          return [
            ...
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 16000 },
            { provider: 'opencode', model: 'nemotron-3-ultra-free', timeout: 8000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 4000 }
          ];
        }
        if (t.includes('codex') || t.includes('antigravity')) {
          return [
            ...
            { provider: 'ollama', model: 'nemotron-3-ultra', timeout: 20000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 15000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 6000 }
          ];
        }
        if (t.includes('minimax') || t.includes('m3')) {
          return [
            ...
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 8000 },
            { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 4000 }
          ];
        }
      }
      ...
      return [
        ...
        // Tier 3: OpenRouter Nemotron-3-Ultra-550B
        { provider: 'openrouter', model: 'nvidia/llama-3.1-nemotron-70b-instruct:free', timeout: 14000 },
    ...
    // Preserves 100% token quota (zero wasted parallel calls) and guarantees full execution under 52s budget.
    // ========================================================================
